...
<nav><a class="active" href="cover.php">Ana Sayfa</a><a href="locateus.php">Bize Ulaşın</a></nav>
</div>
</header>
...
<button type="button" class="button" id="openLoginButton">Giriş Yap</button>
<button type="button" class="button secondary" id="openSignupButton">Yeni Hesap Oluştur</button>
</div>
</section>
</main>
<footer class="landing-footer"><a href="admin/alogin.php">Yönetici Girişi</a><a href="admin/mlogin.php">Manager Girişi</a><a href="Admin/dlogin.php">Doktor Girişi</a></footer>
...
<div class="modal-head"><h2 id="loginTitle">Hoş geldiniz</h2><button type="button" class="close" id="closeLoginButton" aria-label="Kapat">&times;</button></div>
<div class="modal-body"><p class="login-note">Hasta hesabınızla giriş yaparak randevularınızı yönetin.</p>
<?php if($error!==''):?><div class="alert error login-error" role="alert"><?php echo htmlspecialchars($error,ENT_QUOTES,'UTF-8');?></div><?php endif;?>
<div class="field"><label for="uname">Kullanıcı adı</label><input id="uname" type="text" name="uname" placeholder="Kullanıcı adınız" autocomplete="username" required></div>
<div class="field" style="margin-top:16px"><label for="psw">Şifre</label><input id="psw" type="password" name="psw" placeholder="Şifreniz" autocomplete="current-password" required></div>
</div>
<div class="modal-footer"><button type="button" class="button secondary" id="cancelLoginButton">Vazgeç</button><button type="submit" name="login" class="button">Giriş Yap</button></div>
</form>
</div>
...
<div class="modal-head"><h2 id="signupTitle">Yeni hesap oluştur</h2><button type="button" class="close" id="closeSignupButton" aria-label="Kapat">&times;</button></div>
<div class="modal-body signup-copy"><img class="steps-image" src="images/steps.png" alt="Randevu oluşturma adımları"><p><strong>Hesabınızı oluşturun → Tarihinizi seçin → Randevunuzu oluşturun</strong></p><p>Randevu almak hiç bu kadar kolay olmamıştı.</p></div>
<div class="modal-footer"><button type="button" class="button secondary" id="cancelSignupButton">Vazgeç</button><a href="signup.php" class="button">Kayıt Ol</a></div>
</div>
</div>
...
(function () {
    ...
    var loginModal = byId('loginModal');
    var signupModal = byId('signupModal');

    byId('openLoginButton').addEventListener('click', function () {
        openModal(loginModal);
    });

    byId('openSignupButton').addEventListener('click', function () {
        openModal(signupModal);
    });

    byId('closeLoginButton').addEventListener('click', function () {
        closeModal(loginModal);
    });

    byId('cancelLoginButton').addEventListener('click', function () {
        closeModal(loginModal);
    });

    byId('closeSignupButton').addEventListener('click', function () {
        closeModal(signupModal);
    });

    byId('cancelSignupButton').addEventListener('click', function () {
        closeModal(signupModal);
    });
